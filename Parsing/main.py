from bs4 import BeautifulSoup
from urllib.parse import urlparse
import time
import requests
import logging
import collections
from selenium import webdriver

logging.basicConfig(level=logging.DEBUG)
logger = logging.getLogger('wb')

ParseEsult = collections.namedtuple(
    'Parseresult',
    (
        'name',
        'url'
    ),
)
# class Bot:
#     def __init__(self):
#         self.driver = webdriver.Firefox(executable_path=r'C:\Intel\geckodriver.exe')
#         self.navigate()
#
#     def navigate(self):
#         self.driver.get('https://www.pinterest.com/pin/735986764087357271/')

class Client:
    def __init__(self):
        self.driver = webdriver.Firefox(executable_path=r'C:\Intel\geckodriver.exe')
        #####  !!!!!!!!!!!!!  self.navigate()
        # self.session = requests.Session()
        # self.session.headers = {
        #     'user-agent: Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/81.0.4044.17 Safari/537.36 OPR/68.0.3609.0 (Edition developer)'
        # }
        # self.result = []
        self.text = self.driver.page_source
        #self.images = self.driver.find_element_by_xpath("//div[@id='__PWS_ROOT__']//div[@class='gridCentered']//img").get_attribute()
        #self.images = self.driver.find_element_by_xpath("//div[@id='__PWS_ROOT__']").find_element_by_xpath("//div[@class='gridCentered']").find_element_by_tag_name('img').text


    # def navigate(self):
    #     self.driver.get('https://www.pinterest.com/pin/735986764087357271/')
    #     time.sleep(5)
    #     return self.driver.title

    def load_page(self, page: int = None):
            url = 'http://rst.ua/autosalons/'
            res = self.session.get(url=url)

            # res = self.session.get(url=url,cookies =
            # {'_pinterest_sess':''})
            print(res.status_code)
            res.raise_for_status()
            return res.text

    def parse_page(self, text: str):
        print('============parse_page========')
        #soup = bs4.BeautifulSoup(text, 'lxml')
        soup = BeautifulSoup.BeautifulSoup(self.text, 'html.parser')
        container = soup.select('div.__PWS_ROOT__')
        print(container)
        #container = soup.select("//div[@id='__PWS_ROOT__']//div[@class='gridCentered']//img")
        # for i in self.images:
        #     print(i)
        # for each_div in soup.findAll('div',{'class':'Grid__Container'}):
        #     print(each_div)



        #soup.('a') # [<a ..>, ..]
        # d = soup.find_all('div', class='XiG zI7 iyn Hsu')

        # for link in soup.find_all('img'):
        #     print(link.get('href'))
        # soup.find_all(href=re.compile("elsie"), id='link1')
        # container = soup.find_all('div', {'class': ['Yl-', 'MIw','Hb7']})
        # print(container)
        # for block in container:
        #     self.parse_block(block=block)

    def parse_block(self,block):
        # logger.info(block)
        # logger.info('='*100)

        #url_block = block.select_one()
        url_block = block.select_one('img.hCL kVc L4E MIw')
        if not url_block:
            logger.error('no url_block')
            return

        url = url_block.get('href')
        if not url:
            logger.error('no href')
            return

        logger.info('%s', url)

    def run(self):
        # head = self.navigate()
        text = self.load_page()
        print(text)
        self.parse_page(text=self.text)

    def quit(self):
        time.sleep(50)
        self.driver.quit()

url = 'http://rst.ua/autosalons/'
#url = 'https://www.olx.ua/transport/legkovye-avtomobili/q-электромобиль/'

def get_html(url):
    r = requests.get(url)
    return r.text

def get_ip(html):
    soup = BeautifulSoup(html, 'lxml')
    container = soup.select('div.offer-wrapper')
    #container = soup.select(".//div[@class='offer-wrapper']//td[@class='photo-cell']//img")
    for g in container:
        images = g.find_all('img', class_ = "fleft")

    for g in container:
        link1 = g.find_all('a')
        url = [link['href'] for link in link1 if link.get('href')]
        print(url)
        # nu = urlparse(url)
        # print(nu)
        #link = g.find_all('td', class_ = "title-cell ")/div/
    # for cinema in city.xpath("//td[@class='photo-cell']//img"):
    #     print cinema.xpath("text()")[0] # Name
    #     print cinema.xpath("@href")[0] # url
    #print(container)
    list_div = soup.find_all('div', class_ = "offer-wrapper")
    #print(list_div)
    img = soup.find('div', class_ = "offer-wrapper").find_all_next('img', class_ = "fleft")

    ipp = soup.find('table', id = "offers_table").find_all_next('div', class_ = 'space rel')
    ipp = soup.find('div', class_ = "offer-wrapper").find_all_next('div', class_ = 'space rel')
    #iip = soup.find('div', class_ = "offer-wrapper").
    print(img)


if __name__ == '__main__':
    html = get_html(url)
    get_ip(html)
    print('retyrtyu')
    # parser = Client()
    # parser.run() #https://ru.traveltables.com/img/flags_gifs/
    # parser.quit()

